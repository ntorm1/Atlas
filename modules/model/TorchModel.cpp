...
//============================================================================
TorchModelConfig::TorchModelConfig(SharedPtr<ModelConfig> base_config,
                                   String torch_script_file) noexcept
    : m_torch_script_file(torch_script_file), m_base_config(base_config) {}
...
//============================================================================
TorchModel::TorchModel(String id,
  ...
  try {
    ...
    throw AtlasException(
        std::format("Error loading the torch script file: {}", e.what()));
  }
}

//============================================================================
TorchModel::~TorchModel() noexcept { delete m_impl; }

//============================================================================
HashMap<String, Vector<float>> TorchModel::namedParameters() const noexcept {
  return namedParamsToHashMap(m_impl->torch_module.named_parameters());
}

//============================================================================
void TorchModel::train() noexcept {
  ...
  // init torch tensors
  auto X_train =
      torch::zeros({training_window - look_forward, x_cols}, torch::kF32);
  ...
  LinAlg::EigenMap<LinAlg::EigenMatrixXf> X_train_eigen(
      X_train_data, training_window - look_forward, x_cols);
  LinAlg::EigenMap<LinAlg::EigenVectorXf> y_train_eigen(
      y_train_data, training_window - look_forward);
  copyBlocks<float, LinAlg::EigenMatrixXf, LinAlg::EigenVectorXf>(
      X_train_eigen, y_train_eigen);

  trainTorchModule(m_impl->torch_module, X_train, y_train);
}

//============================================================================
void TorchModel::reset() noexcept {}
