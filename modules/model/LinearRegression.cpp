...
//============================================================================
static void calculatePValues(LinAlg::EigenMatrixXd const &X,
                             LinAlg::EigenVectorXd const &y,
                             LinAlg::EigenVectorXd const &theta,
                             LinAlg::EigenVectorXd &p_values) noexcept {
  // Calculate residuals
  ...
  for (int i = 0; i < theta.size(); i++) {
    // Calculate p-value using cumulative distribution function of Student's
    // t-distribution
...
//============================================================================
static void gramSchmidtOrthogonalization(LinAlg::EigenMatrixXd &A) {
...
//============================================================================
static LinAlg::EigenVectorXd lassoRegression(LinAlg::EigenMatrixXd const &X,
                                             LinAlg::EigenVectorXd const &y,
...
//============================================================================
static LinAlg::EigenVectorXd ridgeRegression(LinAlg::EigenMatrixXd const &X,
                                             LinAlg::EigenVectorXd const &y,
                                             double alpha) noexcept {
  // Compute coefficients using closed-form solution
  int n_features = static_cast<int>(X.cols());
  LinAlg::EigenMatrixXd I =
      LinAlg::EigenMatrixXd::Identity(n_features, n_features);
  LinAlg::EigenVectorXd coef =
      (X.transpose() * X + alpha * I).ldlt().solve(X.transpose() * y);
  return coef;
}

//============================================================================
LinearRegressionModelConfig::LinearRegressionModelConfig(
    SharedPtr<ModelConfig> base_config, LinearRegressionSolver solver) noexcept
    : m_solver(solver), m_base_config(std::move(base_config)) {}

//============================================================================
LassoRegressionModelConfig::LassoRegressionModelConfig(
    SharedPtr<ModelConfig> base_config, double alpha, double epsilon,
    size_t max_iter) noexcept
    : LinearRegressionModelConfig(std::move(base_config),
                                  LinearRegressionSolver::Lasso),
      m_alpha(alpha), m_epsilon(epsilon), m_max_iter(max_iter) {}
...
void LinearRegressionModel::train() noexcept {
  ...
  case LinearRegressionSolver::Lasso: {
    auto config =
        static_cast<LassoRegressionModelConfig const *>(m_lr_config.get());
    m_theta = lassoRegression(m_X_train, m_y_train, config->m_alpha,
                              config->m_epsilon, config->m_max_iter);
  } break;
  }
  if (m_pvalues.size()) {
...
//============================================================================
void LinearRegressionModel::reset() noexcept {
  m_X.setZero();
  m_y.setZero();
  m_theta.setZero();
}
...
//============================================================================
void LinearRegressionModel::predict() noexcept {
