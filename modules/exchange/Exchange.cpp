...
//============================================================================
Result<bool, AtlasException> Exchange::validate() noexcept {
...
//============================================================================
Result<bool, AtlasException> Exchange::build() noexcept {
  ...
  // timestamp
  m_impl->returns_scalar.resize(m_impl->assets.size());
...
void Exchange::reset() noexcept {
  ...
  m_impl->registered_triggers.erase(
      std::remove_if(m_impl->registered_triggers.begin(),
  ...
  for (auto it = m_impl->ast_cache.begin(); it != m_impl->ast_cache.end();
       /* no increment here */) {
  ...
  m_impl->asset_observers.erase(std::remove_if(m_impl->asset_observers.begin(),
  ...
  m_impl->models.erase(std::remove_if(m_impl->models.begin(),
                                      m_impl->models.end(),
                                      [](const auto &model) {
...
//============================================================================
void Exchange::enableCache() noexcept {
...
void Exchange::step(Int64 global_time) noexcept {
  ...
  std::for_each(m_impl->registered_triggers.begin(),
                m_impl->registered_triggers.end(), [](auto &trigger) {
                  assert(trigger);
                  trigger->step();
                });
  m_impl->current_index++; // cov node valls currentIdx on first step
  std::for_each(m_impl->covariance_nodes.begin(),
                m_impl->covariance_nodes.end(),
                [](auto &node_pair) { node_pair.second->evaluate(); });

  // update the cache of any asset observers
  std::for_each(m_impl->asset_observers.begin(), m_impl->asset_observers.end(),
                [](auto &observer) { observer->cacheBase(); });
  std::for_each(m_impl->models.begin(), m_impl->models.end(),
...
//============================================================================
void Exchange::registerModel(SharedPtr<Model::ModelBase> model) noexcept {
  auto const &id = model->getId();
  m_impl->models[id] = model;
}

//============================================================================
SharedPtr<AST::AssetObserverNode> Exchange::registerObserver(
...
//============================================================================
void Exchange::cleanupCovarianceNodes() noexcept {
  ...
  for (auto it = m_impl->covariance_nodes.begin();
       it != m_impl->covariance_nodes.end();) {
    auto &[id, node] = *it;
    node->reset();
  ...
  // Remove nodes from covariance_nodes and registered_triggers if they are not
  // being used
  for (const auto &key : keysToRemove) {
    ...
    // delete node to clear up trigger use count. If the use count is equal to
    // 2, i.e. the current node and the trigger held by the exchange then we can
    // remove the trigger
    node.reset();
    if (trigger.use_count() == 2) {
      m_impl->registered_triggers.erase(
          std::remove(m_impl->registered_triggers.begin(),
                      m_impl->registered_triggers.end(), trigger),
          m_impl->registered_triggers.end());
    }
  }
}
...
//============================================================================
void Exchange::cleanupTriggerNodes() noexcept {
  for (auto it = m_impl->registered_triggers.begin();
       it != m_impl->registered_triggers.end();) {
    auto trigger = *it;
    if (trigger.use_count() == 1) {
...
//============================================================================
void Exchange::enableNodeCache(String const &name,
...
//============================================================================
Option<String> Exchange::getDatetimeFormat() const noexcept {
...
//============================================================================
size_t Exchange::currentIdx() const noexcept {
  assert(m_impl->current_index > 0);
...
Exchange::getCovarianceNode(String const &id,
                            SharedPtr<AST::TriggerNode> trigger,
                            size_t lookback, CovarianceType type) noexcept {
  ...
  auto it = std::find(m_impl->registered_triggers.begin(),
                      m_impl->registered_triggers.end(), trigger);
...
Exchange::getMarketReturnsBlock(size_t start_idx,
  ...
  return m_impl->returns(Eigen::all, Eigen::seq(start_idx, end_idx));
}
...
//============================================================================
size_t Exchange::getExchangeOffset() const noexcept {
...
//============================================================================
HashMap<String, size_t> const &Exchange::getHeaders() const noexcept {
