...
//============================================================================
bool StrategyNode::evaluate(Eigen::Ref<Eigen::VectorXd> target) noexcept {
...
//============================================================================
void StrategyNode::setCommissionManager(
...
//============================================================================
void StrategyNode::enableCopyWeightsBuffer() noexcept {
...
//============================================================================
LinAlg::EigenRef<LinAlg::EigenVectorXd> StrategyNode::getPnL() noexcept {
  return m_allocation->getPnL();
}

//============================================================================s
Exchange &StrategyNode::getExchange() noexcept {
  return m_allocation->getExchange();
}

//============================================================================
size_t StrategyNode::refreshWarmup() noexcept {
  m_warmup = m_allocation->refreshWarmup();
  return m_warmup;
}

//============================================================================
void StrategyNode::reset() noexcept {
  if (m_trigger) {
    (*m_trigger)->reset();
  }
  m_allocation->reset();
...
//============================================================================
void StrategyNode::setTracer(SharedPtr<Tracer> tracer) noexcept {
...
StrategyNode::make(SharedPtr<AllocationBaseNode> allocation
  ...
  if (use_count > 3) // pybind11 instance + this
  {
    throw std::runtime_error(
        "Py Allocation node use count expected < 3, found " +
...
//============================================================================
void StrategyNode::setTrigger(SharedPtr<TriggerNode> trigger) noexcept {
  m_trigger = trigger;
}
...
FixedAllocationNode::FixedAllocationNode(
    Vector<std::pair<size_t, double>> m_allocations, Exchange *exchange,
    double epsilon) noexcept
    : AllocationBaseNode(AllocationType::FIXED, *exchange, epsilon,
                         std::nullopt),
...
FixedAllocationNode::make(Vector<std::pair<String, double>> allocations,
                          Exchange *exchange, double epsilon) noexcept {
  ...
  return std::make_unique<FixedAllocationNode>(std::move(allocations_ids),
                                               exchange, epsilon);
}

//============================================================================
SharedPtr<FixedAllocationNode>
FixedAllocationNode::pyMake(Vector<std::pair<String, double>> m_allocations,
...
//============================================================================
void FixedAllocationNode::evaluateChild(
