...
//============================================================================
void TradeLimitNode::reset() noexcept { m_is_first_step = true; }

//============================================================================
TradeLimitNode::~TradeLimitNode() noexcept {}

//============================================================================
TradeLimitNode::TradeLimitNode(AllocationBaseNode *parent,
                               TradeLimitType trade_type, double limit) noexcept
...
void TradeLimitNode::evaluate(
    ...
    LinAlg::EigenRef<LinAlg::EigenVectorXd> previous_weights) noexcept {
  // in the first step we can't have pnl as allocation node is evaluated after
  // the portfolio is priced. And it would cause index error trying to get the
  // previous steps returns as we see below.
  ...
  if (m_is_first_step) {
    ...
    // recent evaluation. So we need to update the trade pnl vector to adjust
    // the pnl Stat by pulling in the previous market returns using index offset
    // and making a mutable copy of the view
    ...
    // we zero out the weights when the pnl is less than the limit. For take
    // profit we zero out the weights when the pnl is greater than the limit.
    // Add an additional comparison to 0.0 to prevent the new weights from being
    // zeroed out beforehand
    if (isTradeTypeSet(TradeLimitType::STOP_LOSS)) {
      current_weights = current_weights.array().cwiseProduct(
          ((pnl.array() > m_impl->m_stop_loss) || (pnl.array() == 0.0))
              .cast<double>());
    }
    if (isTradeTypeSet(TradeLimitType::TAKE_PROFIT)) {
      current_weights = current_weights.array().cwiseProduct(
          ((pnl.array() < m_impl->m_take_profit) || (pnl.array() == 0.0))
              .cast<double>());
    }
  }
  ...
  pnl = ((current_weights.array() * previous_weights.array() < 0.0f) ||
         (previous_weights.array() == 0) && (current_weights.array() != 0))
            .select(1.0f, pnl);

  // update closed trade. If the current weight is 0 and the previous weight is
  // not 0 then zero out the pnl vector
  pnl = ((current_weights.array() == 0) && (previous_weights.array() != 0))
            .select(0.0f, pnl);
}

//============================================================================
double TradeLimitNode::getStopLoss(SharedPtr<TradeLimitNode> node) noexcept {
...
//============================================================================
double TradeLimitNode::getTakeProfit(SharedPtr<TradeLimitNode> node) noexcept {
...
//============================================================================
void TradeLimitNode::setStopLoss(SharedPtr<TradeLimitNode> node,
                                 double stopLoss) noexcept {
  if (stopLoss == 0.0) {
...
//============================================================================
void TradeLimitNode::setTakeProfit(SharedPtr<TradeLimitNode> node,
                                   double takeProfit) noexcept {
  if (takeProfit == 0.0) {
...
//============================================================================
void TradeLimitNode::setLimit(TradeLimitType trade_type,
