...
TriggerNode::registerNode(SharedPtr<TriggerNode> &&node) noexcept {
  return const_cast<Exchange &>(node->getExchange())
      .registerTrigger(std::move(node)); // TODO: ugly hack
}

//============================================================================
TriggerNode::TriggerNode(Exchange const &exchange) noexcept
    : m_exchange(exchange), ExpressionNode<bool>(NodeType::STRATEGY_RUNNER) {}

//============================================================================
StrategyMonthlyRunnerNode::StrategyMonthlyRunnerNode(Exchange const &exchange,
                                                     bool eom_trigger) noexcept
    : TriggerNode(exchange), m_eom_trigger(eom_trigger) {}

//============================================================================
Result<bool, AtlasException> StrategyMonthlyRunnerNode::build() noexcept {
...
//============================================================================
void StrategyMonthlyRunnerNode::step() noexcept { m_index_counter++; }

//============================================================================
void StrategyMonthlyRunnerNode::reset() noexcept { m_index_counter = 0; }

//============================================================================
Result<bool, AtlasException> PeriodicTriggerNode::build() noexcept {
  auto const &timestamps = m_exchange.getTimestamps();
  m_tradeable_mask.resize(timestamps.size());
  m_tradeable_mask.setZero();
  for (size_t t = 0; t < timestamps.size(); ++t) {
...
//============================================================================
void PeriodicTriggerNode::step() noexcept { m_index_counter++; }

//============================================================================
PeriodicTriggerNode::PeriodicTriggerNode(Exchange const &exchange,
                                         size_t frequency) noexcept
    : TriggerNode(exchange), m_frequency(frequency) {}

//============================================================================
void PeriodicTriggerNode::reset() noexcept { m_index_counter = 0; }

//============================================================================
bool PeriodicTriggerNode::evaluate() noexcept {
...
//============================================================================
SharedPtr<TriggerNode> PeriodicTriggerNode::make(SharedPtr<Exchange> exchange,
                                                 size_t frequency) {
...
//============================================================================
bool StrategyMonthlyRunnerNode::evaluate() noexcept {
  assert((m_index_counter - 1) < static_cast<size_t>(m_tradeable_mask.size()));
  return static_cast<bool>(m_tradeable_mask(m_index_counter - 1));
}
...
//============================================================================
SharedPtr<TriggerNode>
StrategyMonthlyRunnerNode::make(SharedPtr<Exchange> exchange,
                                bool eom_trigger) {
