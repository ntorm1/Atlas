...
//============================================================================
AllocationBaseNode::AllocationBaseNode(AllocationType m_type,
                                       Exchange &exchange, double epsilon,
...
//============================================================================
double AllocationBaseNode::getAllocEpsilon() const noexcept {
  return m_impl->m_epsilon;
}

//============================================================================
size_t AllocationBaseNode::getAssetCount() const noexcept {
...
//============================================================================
void AllocationBaseNode::setWeightScale(
    SharedPtr<AllocationWeightNode> scale) noexcept {
...
//============================================================================
void AllocationBaseNode::setTradeLimit(TradeLimitType t,
...
//============================================================================
void AllocationBaseNode::setCommissionManager(
    SharedPtr<CommisionManager> manager) noexcept {
  copy_weights_buffer = true;
  m_impl->m_commision_manager = manager;
}

//============================================================================
void AllocationBaseNode::setTracer(SharedPtr<Tracer> tracer) noexcept {
...
//============================================================================
LinAlg::EigenRef<LinAlg::EigenVectorXd> AllocationBaseNode::getPnL() noexcept {
  return m_impl->m_tracer->getPnL();
}

//============================================================================
void AllocationBaseNode::evaluate(
  ...
  if (m_impl->m_weight_scale && !(*m_impl->m_weight_scale)->getIsCached()) {
    return;
  }
  ...
  // if we have a weight scale node we need to evaluate it over the generated
  // weights
  ...
  if (m_impl->m_epsilon > 0) {
    target = ((target - m_impl->m_tracer->m_weights_buffer).cwiseAbs().array() <
              m_impl->m_epsilon)
  ...
  // if we have stop loss or take profit we need to check if the trade limits
  // have been exceeded by passing in the previous weights to compute the pnl,
  // then adjust target weights accordingly
  if (m_impl->m_trade_limit) {
    (*m_impl->m_trade_limit)
        ->evaluate(m_impl->m_tracer->getPnL(), target,
  ...
  if (m_impl->m_commision_manager &&
      (*m_impl->m_commision_manager)->hasCommission()) {
    (*m_impl->m_commision_manager)
        ->calculateCommission(target, m_impl->m_tracer->m_weights_buffer);
  }
}

//============================================================================
AllocationNode::AllocationNode(SharedPtr<StrategyBufferOpNode> exchange_view,
                               AllocationType type, Option<double> alloc_param,
                               double epsilon) noexcept
    : AllocationBaseNode(type, exchange_view->getExchange(), epsilon,
                         alloc_param),
...
//============================================================================
size_t AllocationNode::refreshWarmup() noexcept {
...
AllocationNode::make(SharedPtr<StrategyBufferOpNode> exchange_view,
                     AllocationType type, Option<double> alloc_param,
                     double epsilon) noexcept {
  ...
  auto node = std::make_unique<AllocationNode>(std::move(exchange_view), type,
                                               alloc_param, epsilon);
  ...
  if (type == AllocationType::NEXTREME) {
    ...
    // attempt to cast the alloc_param to a size_t to use as the number of
    // assets to allocate ussing n extreme values
    try {
    ...
    if (node->getNAllocParam() * 2 >= asset_count) {
      return Err("Allocation type NEXTREME requires a parameter less than half "
                 "the number of assets in the exchange");
    }
  }
...
AllocationNode::pyMake(SharedPtr<StrategyBufferOpNode> exchange_view,
                       AllocationType type, Option<double> alloc_param,
                       double epsilon) {
...
//============================================================================
void AllocationNode::evaluateChild(
    LinAlg::EigenRef<LinAlg::EigenVectorXd> target) noexcept {
  ...
  case AllocationType::CONDITIONAL_SPLIT: {
    ...
    target =
        (target.array() < *m_impl->m_alloc_param)
            .select(
                -c,
                (target.array() > *m_impl->m_alloc_param).select(c, target));
